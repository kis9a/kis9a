(() => {
  ...
  var Link = (name, opts = { as: "", active: true }) => {
    let item = routes.find((v) => v.name == name);
    if (!item) {
      item = { name, href: "#" };
    }
    return h("a", { class: `link ${opts.active ? "" : "disable"}`, href: item.href }, text(opts.as || item.name.toUpperCase()));
  };
  ...
  // sources/src/components/link/unit.js
  var viewLink = () => {
    return Link("name");
  };
  var viewLinkActive = () => {
  ...
  var Toast = (msg, {
    ...
    switch (status) {
      case "err":
        toast2.style.background = "#e93c00";
        svg = exclamation_default;
        break;
      case "suc":
        toast2.style.background = "#68c39f";
        svg = check_circle_default;
        break;
      default:
        toast2.style.background = bg;
    }
    toast2.innerHTML = svg;
    toast2.style.bottom = bottom;
    toast2.style.left = left;
    toast2.style.right = right;
    toast2.style.top = top;
    toast2.style.width = width;
    toast2.style.minWidth = minWidth;
    toast2.style.height = height;
    toast2.style.display = "flex";
    toast2.style.alignItems = "center";
    toast2.style.position = "fixed";
    toast2.style.margin = "10px";
    toast2.style.padding = "0.8rem 1.6rem";
    toast2.style.fontSize = "1.8rem";
    toast2.style.color = "inherit";
    toast2.style.borderRadius = "0.5rem 0rem 0rem 0rem";
    toast2.style.border = "1px solid #d0c9c3";
    toast2.style.boxShadow = "0 0 2px 2px #f2f2f2";
    const svgNode = toast2.childNodes[0];
    ...
    toast2.appendChild(document.createTextNode(msg));
  ...
  // sources/src/components/toast/unit.js
  var viewToast = () => {
  ...
  var toastError = (state) => {
    Toast("error", { status: "err" });
    return { ...state };
  };
  var viewToastSuccess = () => {
    return h("button", { onclick: toastSuccess }, text("success"));
  };
  var toastSuccess = (state) => {
    Toast("success", { status: "suc" });
    return { ...state };
  };

  // sources/src/components/header/unit.js
  var viewHeader = () => {
    return Header();
  };
  ...
  var setView = (state, views) => {
    const view = h("div", {}, views.map((v) => h("div", {}, h("div", {}, v.view()))));
    return { ...state, cview: view };
  };
  app({
    init: initialState,
    view: ({ cview }) => h("div", { class: "container" }, [
      Header(),
      h("main", { class: "main" }, [
        h("div", { class: "sidebar" }, sides && sides.map((v) => h("div", { class: "side", onclick: [setView, v.views] }, text(v.name)))),
