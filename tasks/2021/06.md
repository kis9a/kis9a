...
- [x] ~~/error log top level change and return err.~~
      ⬆ - [x] ~~commander.Command.Run method's error log is subtle, fork and fix. private.~~ -> migreate flag pkg.
...
```
check this working walker and watcher.
...
terraform subcommand and casename change to flag name.
...
function install-git-ps() {
  cp $INFRA_BUILD_DIR/etc/dotfiles/bin/git-ps /bin/git-ps
}
...
if [[ ! -f /bin/git-ps ]]; then
  install-git-ps
fi
```

cp $GOPATH/bin/go1.16.4 ./go
