...
- [x] /jnfrabuilder directory structure changed /infrabuilder/ web/...|infra/ cmd|envs|modules
- [x] research TUI applicatoin golang.
- [x] /infrabuilder environment reference with struct. and change variables name.
- [x] create default tmpls and json.
- [x] golang create template output function.
- [x] golang template useage.
- [x] golang exporter json.
- [x] default.variables.tf, env.variables.tf.
- [x] as a terraform command line interface. reference <https://github.com/hashicorp/terraform-exec/>
...
-> esbuild javascript to /kis9a{{{

- [x] esbuild initialized and test bundle.
- [x] only build in pagese/\*.js, css, html.
- [x] create scheme new go packge to more simple interface hyperapp and esbuild like hugo. -> project name : hh}}}
...
frontend preformance<!--{{{-->

- インターネット速度 平均
- インターネット回線の平均速度を比較
- 回線タイプ 下り平均速度 上り平均速度 平均 ping 値
- 光回線 下り:194.7Mbps 上り:168.2Mbps 27.2ms
- ポケット Wi-Fi 下り:21.6Mbps 上り:7.1Mbps 86.0ms
- ホームルーター 下り:24.5Mbps 上り:4.3Mbps

- ユーザーの通信回線が 3G 回線(1Mpbs 程度）であれば、転送量 100~200KB 程度削減すると、通信時間が１秒位短くなる
- 通信回線が４ G 回線(10~20Mbps)であれば、転送量 1~2MB 程度の削減が、通信時間１秒の短縮に対応する
- 通信回線が固定回線(500M~1Gbps)だれば、転送量 50~100MB 程度の削減が、通信時間１秒に短縮に対応する
<!--}}}-->
...
- [ ] code reading arround ts-notify [GitHub - OrlovEvgeny/TinyJPG: images jpg or jpeg compressed and watcher fsnotify](https://github.com/OrlovEvgeny/TinyJPG)
...
adfadf, asdfadf ,adsf adfadf, adf,adf ,adsf
adfadf, asdfadf ,adsf adfadf, adf,adf ,adsf
adfadf, asdfadf ,adsf adfadf, adf,adf ,adsf
adfadf, asdfadf ,adsf adfadf, adf,adf ,adsf
...
- [ ] html generate step function, by template. and template took some value.
- [ ] css bundle step read <https://esbuild.github.io/content-types/#css>

一番大事なコミュニケーションの相手は自分.

/saveAndRun hook conf structure change
dir [ ]string -> path string because [], cmd []string -> string

アスタリスク\*はセパレータ以外のシーケンスを意味する。
ハテナ?はセパレータ以外の１文字を意味する。
linux では\\はエスケープ文字として使われるが、windows ではセパレータとして認識される。
